...
export default function ArtistLinksForm({ artistId, existingLinks }: { artistId: string, existingLinks: any[] }) {
    const supabase = createClient();
    const router = useRouter();
    const [loading, setLoading] = useState(true);
    const [links, setLinks] = useState(existingLinks || []);
    const [newLinks, setNewLinks] = useState<any[]>([]);
    const [editedLinks, setEditedLinks] = useState<Record<string, Partial<ArtistLink>>>({});
    ...
    const form = useForm({
        resolver: zodResolver(artistLinkSchema),
        defaultValues: {
    ...
    function handleEdit(linkId: string, field: keyof ArtistLink, value: string) {
        setEditedLinks(prev => ({
            ...prev,
    ...
    async function handleSave(link: ArtistLink, isNew = false) {
        try {
            setLoading(true);
            ...
            toast.success("Artist link saved!");
            router.refresh();
        } catch (error: any) {
    ...
    async function handleDelete(linkId: string, isNew = false) {
        try {
            ...
            if (isNew) {
                ...
                const { error } = await supabase.from("artist_links").delete().eq("id", linkId);
            ...
            toast.success("Artist link deleted!");
            router.refresh();
        } catch (error: any) {
            toast.error(error.message);
        } finally {
    ...
    function addNewLink() {
        setNewLinks([...newLinks, { id: Date.now().toString(), artist_id: artistId, platform_name: "", url: "" }]);
    }
